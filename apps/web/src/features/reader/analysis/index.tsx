  ...
  return (
    <main className="min-h-screen bg-[#f5fbf7] text-[#071b14]">
      ...
      <div className="mx-auto max-w-6xl px-5 py-8 md:py-10">
        <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
          <article className="overflow-hidden rounded-lg border border-[#d9e9df] bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
              <p className="max-w-2xl text-sm leading-7 text-[#5e7168]">{featuredReport.description}</p>
        ...
        <section className="mt-5 rounded-lg border border-[#d9e9df] bg-white p-5 text-sm leading-7 text-[#5e7168] shadow-sm">
          {content.summary}
        </section>

        <section className="mt-10 grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
          <div>
            <p className="text-xs font-black uppercase text-[#0b9b5b]">{content.heroBadge}</p>
            <h2 className="mt-2 text-2xl font-black">{content.thesisHeading}</h2>
            <p className="mt-2 max-w-3xl text-sm leading-7 text-[#5e7168]">{content.thesisDescription}</p>
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {content.theses.map((thesis) => (
                <article className="overflow-hidden rounded-lg border border-[#d9e9df] bg-white shadow-sm" key={thesis.title}>
                  <div className={`${thesisToneClass[thesis.tone]} flex min-h-24 items-center justify-between gap-4 px-5 py-5`}>
                    ...
                    <span className="rounded-full bg-white/70 px-3 py-1 text-xs font-bold">{thesis.confidence}</span>
                  </div>
                  <div className="p-5">
                    <h3 className="font-black">{thesis.title}</h3>
                    <p className="mt-2 text-sm leading-6 text-[#60736a]">{thesis.description}</p>
                  </div>
                </article>
              ))}
            </div>
          ...
          <aside className="space-y-5">
            <section className="rounded-lg border border-[#d9e9df] bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-[#e4f0e9] px-4 py-3">
                ...
                <span className="text-xs font-semibold text-[#0da75b]">{content.indicatorsLinkLabel}</span>
              </div>
              <div className="space-y-4 p-4">
                {content.indicators.map((indicator) => (
                  <article key={indicator.label}>
                    ...
                    <p className="mt-1 text-xs leading-5 text-[#6b7c74]">{indicator.description}</p>
                  </article>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-lg border border-[#d9e9df] bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-[#e4f0e9] px-4 py-3">
                ...
                <span className="text-xs font-semibold text-[#0da75b]">{content.risksLinkLabel}</span>
              </div>
              <div className="space-y-4 p-4">
                {content.risks.map((risk) => (
                  <article className="grid grid-cols-[38px_minmax(0,1fr)] gap-3" key={risk.title}>
                    <span className="flex h-8 w-8 items-center justify-center rounded bg-[#fff5cf] text-xs font-black text-[#8a5a00]">
                      {risk.marker}
                    </span>
                    <div>
                      <h3 className="text-sm font-black">{risk.title}</h3>
                      <p className="mt-1 text-xs leading-5 text-[#6b7c74]">{risk.description}</p>
                    </div>
                  </article>
                ))}
              </div>
        ...
        <section className="mt-10 rounded-lg border border-[#d9e9df] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-4 border-b border-[#e4f0e9] pb-4">
            ...
            <span className="text-xs font-bold text-[#0da75b]">{locale}</span>
          </div>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {content.metrics.map((metric) => (
              <article className="grid grid-cols-[44px_minmax(0,1fr)] gap-3" key={metric.label}>
                <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#0da75b] text-xs font-black text-white">
                  {metric.value}
                </span>
                <div>
